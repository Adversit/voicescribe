...
| 当前实现状态 | 已实现模型列表、下载、删除、预加载主链；当前支持矩阵已移除 `pyannote-3.1`，保留 `campplus-diarization`、`sond-diarization`、`3D-Speaker` 三条分离路径。 |
...
| 当前实现状态 | 已实现样本管理和链路接入；当前说话人链路只维护现有 modelscope 分离路径与 `3D-Speaker` 占位，`3D-Speaker` 的真实加载与转录验收仍未完成。 |
...
### 10.0.1 Typeless 可见处理流水线增量

- 停止录音后，界面必须按真实执行阶段显示转写、润色和输出状态，不能用定时器猜测润色阶段。
- 未启用文本润色时，不显示虚假的“润色中”状态。
- 文本润色 provider 失败或独立处理请求失败时，必须输出并保存原始转写，同时向用户显示降级提示。
...
- 用户可显式开启或关闭应用上下文。
- 开启后，VoiceScribe 只使用录音开始时目标应用的类别，为文本处理提供最小风格提示。
- 首轮不得隐式读取选区、页面正文、完整窗口标题、完整可执行路径或其他应用内容。
- 上下文捕获失败不得阻断录音、转写、文本处理、历史写入或文本输出。
- 应用上下文不得覆盖用户显式选择的 Profile，也不得把口述内容升级为 Agent 执行任务。
