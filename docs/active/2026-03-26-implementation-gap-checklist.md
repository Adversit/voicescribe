...
- `P4-04` 实时转录与历史记录专题：`未完成`
  ...
  - 当前尚未完成侧边栏新增页面、流式时间线、历史记录存储与下载能力

## Feature Topic
- `F1-01` 实时转录页：`未完成`
  - 需基于 `/stream` 桥接实现说话人时间线展示
- `F1-02` 历史记录页：`未完成`
  - 需支持流式和非流式结果自动入库、复制、下载、删除、清空
- `F1-03` 通用页流式传输/AI 摘要/保留音频开关：`未完成`
  - 需建立字段、默认值、约束关系
- `F1-04` 快捷键真实录制：`未完成`
  - 需替代当前数字 keycode 录入，支持单键与左右 `Alt`
