...
## 23. `backend/server.py` 在实时转录专题开发中出现编码污染
- 表现：为 `/history`、`/summary`、`/stream` 补逻辑后，`py_compile` 报错，文件中混入不可见字符与私有区字符。
- 原因：在 Windows 环境下多次局部改写同一大文件时，历史混合编码问题被放大，继续局部修补会连带破坏 docstring 与函数体。
- 处理：从 `git show HEAD:backend/server.py` 恢复基线，再重新整块补回历史记录、摘要与实时流逻辑，并重新跑编译与接口烟测。

## 24. `/stream` 的原始音频契约最初处理错误
- 表现：早期实现把前端发送的原始 PCM 直接当成 `.wav` 文件处理，流式转录链路在真实音频块下不稳定。
- 原因：前后端最初没有统一“Rust 发什么、后端按什么格式转录”的二进制音频契约。
- 处理：Rust 侧 `audio.rs` 统一发 base64 PCM 音频块；前端实时流桥接按块发送；后端通过 `_write_pcm16_wav()` 把 PCM 包装成合法 WAV 再进入转录链路。
